...
public class HashTable<K,V> implements MyMap<K,V>{
	...
	public static void main( String[] args ) {
		AscendingStringComparator Sc = new AscendingStringComparator();
		HashTable<String,Integer> table = new HashTable<String,Integer>(10,Sc);
		table.put( "A", 1 );
		table.put( "B", 2 );
...
// 		System.out.println( table.get( "C" ) );
	...
	public void put(K new_key, V new_value) {
		...
		//We want to iterate the size counter here since we want it to only itereate when something new is created
		this.size++;
